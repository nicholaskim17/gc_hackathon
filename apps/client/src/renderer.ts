...
export class Renderer {
  ...
  constructor(public canvas:HTMLCanvasElement){
    ...
    this.particles=new THREE.InstancedMesh(new THREE.BoxGeometry(1,1,1),new THREE.MeshBasicMaterial({color:'#ffffff',transparent:true,opacity:.85}),120);this.particles.instanceMatrix.setUsage(THREE.DynamicDrawUsage);this.particles.frustumCulled=false;this.scene.add(this.particles);
  ...
  draw(g:Game,t:number,demo=false){
    ...
    if(g.shake&&!this.reducedMotion)this.camera.position.x+=Math.sin(t*100)*g.shake;
    ...
    this.particles.count=Math.min(120,g.particles.length);g.particles.slice(0,120).forEach((p,i)=>{this.temp.position.set(p.x,p.y,p.z);this.temp.rotation.set(t+i,t*2,i);this.temp.scale.setScalar(p.size*Math.min(1,p.life*3));this.temp.updateMatrix();this.particles.setMatrixAt(i,this.temp.matrix);this.particles.setColorAt(i,this.particleColor.set(p.color));});this.particles.instanceMatrix.needsUpdate=true;if(this.particles.instanceColor)this.particles.instanceColor.needsUpdate=true;
    this.dust.rotation.y=t*.005;this.gl.render(this.scene,this.camera);
  }
