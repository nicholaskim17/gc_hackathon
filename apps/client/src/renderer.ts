...
export class Renderer {
  ...
  constructor(public canvas:HTMLCanvasElement){
    ...
    for(let i=0;i<3;i++){
      const ball=new THREE.Mesh(ballGeometry,new THREE.MeshStandardMaterial({color:'#ffe65a',emissive:'#ffc400',emissiveIntensity:1.45,roughness:.24}));ball.castShadow=true;this.ballMeshes.push(ball);this.scene.add(ball);
      const aura=new THREE.Mesh(new THREE.SphereGeometry(.082,14,9),new THREE.MeshBasicMaterial({color:'#ffd52f',transparent:true,opacity:.2,blending:THREE.AdditiveBlending,depthWrite:false}));this.ballAuras.push(aura);this.scene.add(aura);
      const ballLight=new THREE.PointLight('#ffd84a',3.2,2.4,2);this.ballLights.push(ballLight);this.scene.add(ballLight);
      const trail:THREE.Mesh[]=[];for(let n=0;n<12;n++){const m=new THREE.Mesh(ballGeometry,new THREE.MeshBasicMaterial({color:'#ffd94d',transparent:true,opacity:.22*(1-n/12),blending:THREE.AdditiveBlending,depthWrite:false}));trail.push(m);this.scene.add(m);}this.trails.push(trail);
  ...
  draw(g:Game,t:number,demo=false){
    ...
    this.ballMeshes.forEach((mesh,i)=>{const b=g.balls[i];mesh.visible=!!b;this.ballAuras[i].visible=!!b&&!b.extra;this.ballLights[i].visible=!!b&&!b.extra;this.ballShadows[i].visible=!!b;this.fakeRings[i].visible=!!b?.extra;this.fakeLabels[i].visible=!!b?.extra;this.giantHalos[i].visible=!!b&&!b.extra&&g.radius>CONFIG.BALL_RADIUS;if(!b){this.trails[i].forEach(m=>m.visible=false);return;}
      ...
      const mat=mesh.material as THREE.MeshStandardMaterial;mat.color.set(b.extra?'#e16dff':b.smash?'#fff06a':g.radius>CONFIG.BALL_RADIUS?'#ffd83d':'#ffe65a');mat.emissive.set(b.extra?'#3fffe7':b.smash?'#ff9d00':'#ffc400');mat.emissiveIntensity=b.extra?2:b.smash?2:1.45;mat.transparent=b.extra;mat.opacity=b.extra ? .42 : 1;mat.wireframe=b.extra;mat.depthWrite=!b.extra;
      const aura=this.ballAuras[i],ballLight=this.ballLights[i];aura.position.copy(mesh.position);aura.scale.setScalar(g.radius/.065*(1.08+Math.sin(t*9+i)*.09));(aura.material as THREE.MeshBasicMaterial).color.set(b.smash?'#ff9d00':'#ffd52f');ballLight.position.copy(mesh.position);ballLight.color.set(b.smash?'#ff9d00':'#ffd84a');ballLight.intensity=b.smash?5.2:3.2;
