...
export class Tracking{
 ...
 private launchWorker(generation:number,preferred:'GPU'|'CPU'){
  ...
  worker.onmessage=e=>{if(!valid())return;
   ...
   if(e.data.type==='ready'){this.ready=true;this.delegate=e.data.delegate;this.status='Step into view. Keep your head, shoulders and arms visible.';if(this.timeout)clearTimeout(this.timeout);}
