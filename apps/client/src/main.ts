...
function renderGame(){
 ...
 const scoreboard=screen.querySelector<HTMLElement>('.scoreboard')!,roundClock=screen.querySelector<HTMLElement>('.round-clock')!;scoreboard.setAttribute('aria-label','Match score');scoreboard.innerHTML='<span class="stat-label">MATCH SCORE · FIRST TO 7</span><div class="match-score"><div class="score-team"><span>YOU</span><strong id="points-a">0</strong></div><b class="score-divider">:</b><div class="score-team"><span>OPPONENT</span><strong id="points-b">0</strong></div></div>';roundClock.innerHTML='<span class="stat-label">RALLY POINTS</span><strong id="score">0</strong>';
...
function updateHud(){
 const side=session==='network'?network.side:0,yourPoints=side===1?game.points[1]:game.points[0],opponentPoints=side===1?game.points[0]:game.points[1],stats=game.playerStats[side],shownGrade=stats.lastGrade==='GREAT'?'GOOD':stats.lastGrade??'';setText('score',game.score.toLocaleString());setText('points-a',String(yourPoints));setText('points-b',String(opponentPoints));setText('rally',String(game.rally));setText('best',String(game.best));setText('meter-number',`${stats.meter}%`);$('meter-fill').style.width=`${stats.meter}%`;
