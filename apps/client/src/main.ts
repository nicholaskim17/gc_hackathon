...
let paused=false,countdown=0,countBeep=-1,ready=false,last=performance.now(),lastSend=0,lastEffects='',lastPhase='',pauseReason='',missingFor=0;
...
function renderGame(){
 setView('game');lastEffects='';const side=session==='network'?network.side:0;
 screen.innerHTML=`<section class="game-screen"><div class="game-top"><div class="scoreboard"><span class="stat-label">RALLY POINTS</span><strong id="score">0</strong></div><div class="round-clock"><span class="stat-label">FIRST TO 7</span><div class="match-score"><span id="points-a">0</span><small>–</small><span id="points-b">0</span></div></div><div class="rally-stats"><div><span class="stat-label">RALLY</span><strong id="rally">0</strong></div><div><span class="stat-label">BEST</span><strong id="best">0</strong></div><button class="icon-button" id="pause" aria-label="Pause game">${icon('pause')}</button></div></div><div class="effects-bar" id="effects"></div><div class="arena-wrap"><canvas id="arena" aria-label="Full 3D table tennis, viewed from your end. Move your racket to return the approaching ball."></canvas><div class="court-tag"><i></i> THE NIGHT COURT <span>${session==='network'?network.code:session==='practice'?'PRACTICE':'LOCAL DUO'}</span></div><button class="view-button" id="view-toggle">View: Player ${side+1} ↻</button><div class="score-popups" id="score-popups"></div><div class="form-feedback" id="form-feedback" aria-live="polite"></div><div class="serve-notice" id="serve-notice"></div><div class="game-overlay" id="overlay"></div></div><div class="game-bottom"><div class="control-player"><span class="player-dot coral"></span><div><b>${session==='network'?`Player ${side+1} · you`:'Player 1'}</b><span id="control-one">${mode==='camera'?'Move your hand. Swing to return.':'<kbd>W A S D</kbd> move · <kbd>Space</kbd> swing'}</span></div></div><div class="swing-panel"><div><span>POWER METER</span><b id="meter-number">0%</b></div><div class="meter-track"><i id="meter-fill"></i></div><small id="quality-label">Great swings earn power-ups</small></div>${mode==='camera'?previewMarkup(true):''}<div class="control-player right"><div><b>${session==='network'?'Your teammate':session==='practice'?'Practice partner':'Player 2'}</b><span>${session==='shared'?'<kbd>↑ ← ↓ →</kbd> move · <kbd>Enter</kbd> swing':session==='network'?'Opposite screen. Same team.':'Ready for your next shot.'}</span></div><span class="player-dot blue"></span></div></div></section>`;
 canvas();if(renderer)renderer.viewMode=side;if(mode==='camera')mountVideo();$('pause').onclick=()=>pause('Take a breather.');$('view-toggle').onclick=()=>{if(renderer){renderer.viewMode=renderer.viewMode===2?side:2;setText('view-toggle',renderer.viewMode===2?'View: Court ↻':`View: Player ${side+1} ↻`);}};
}
function pause(reason:string){if(view!=='game')return;if(session==='network')network.pause();else paused=true;pauseReason=reason;renderPause(reason);}
function renderPause(reason:string){
 if(!document.getElementById('overlay'))return;
 $('overlay').className='game-overlay visible';$('overlay').innerHTML=`<div class="pause-panel"><span class="eyebrow">TIME OUT</span><h2>${reason.includes('disconnect')||reason.includes('Connection')?'Reconnect. Then rally.':'Take a breather.'}</h2><p id="pause-message"></p><button class="primary" id="resume">Resume ${icon('arrow')}</button>${mode==='camera'?'<button class="secondary" id="switch-keyboard">Continue with keyboard</button><button class="text-button" id="recalibrate">Recalibrate camera</button>':''}<button class="text-button" id="quit">Back to menu</button><p id="resume-status" role="status"></p></div>`;
...
function updateHud(){
 setText('score',game.score.toLocaleString());setText('points-a',String(game.points[0]));setText('points-b',String(game.points[1]));setText('rally',String(game.rally));setText('best',String(game.best));
 const side=session==='network'?network.side:0,stats=game.playerStats[side];setText('meter-number',`${stats.meter}%`);$('meter-fill').style.width=`${stats.meter}%`;
 setText('quality-label',stats.gradeLife>0?`${stats.lastGrade}${stats.lastGrade!=='MISS'&&stats.lastGrade!=='SAVED'?` · ${stats.quality}/100`:''}`:'Great swings earn power-ups');$('quality-label').classList.toggle('perfect',stats.lastGrade==='PERFECT'&&stats.gradeLife>0);
 const award=stats.lastGrade==='PERFECT'?25:stats.lastGrade==='GREAT'?15:stats.lastGrade==='OK'?5:0;
 const gradeText=stats.gradeLife>0?`<strong>${stats.lastGrade==='SAVED'?'SHIELD SAVE!':stats.lastGrade}</strong><small>${stats.lastGrade==='MISS'?'Next ball. You’ve got this.':stats.lastGrade==='SAVED'?'Back in the game.':`+${award} POWER`}</small>`:'';if($('form-feedback').innerHTML!==gradeText){$('form-feedback').innerHTML=gradeText;$('form-feedback').className=`form-feedback ${stats.lastGrade?.toLowerCase()??''}`;}
 const effects=(Object.entries(game.playerEffects[side]) as [Power,number][]).map(([key,duration])=>{const p=POWER[key];return `<span class="active-effect" style="--power:${p.color};--remaining:${duration/p.duration*100}%"><b>${p.icon}</b> ${p.name} <strong>${key==='mega'?`${Math.ceil(duration)}s`:'READY'}</strong></span>`;}).join('')||'<span class="effect-hint">Land on a glowing target. Power up your play. ✦</span>';
...
function drawCountdown(){const num=Math.ceil(countdown);if(num!==countBeep){sound.tone(num?440:880,.12,'sine',.09);countBeep=num;}$('overlay').className='game-overlay visible countdown';$('overlay').innerHTML=`<div class="countdown-card"><span>${mode==='camera'?'YOUR HAND IS YOUR RACKET':'WASD TO MOVE · SPACE TO SWING'}</span><strong>${num||'GO!'}</strong><p>Same team. Keep it flying.</p></div>`;}
